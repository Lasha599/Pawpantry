...
import { useStore, daysUntil, addWeeks } from '@/lib/store';
                      ...
                      <div className="flex flex-wrap gap-2">
                        <button
                          onClick={() => update(s => ({
                            ...
                            subscriptions: s.subscriptions.map(x =>
                              x.id === sub.id
                                ? { ...x, nextDeliveryISO: addWeeks(x.nextDeliveryISO, x.frequencyWeeks) }
                                : x
                            ),
                          }))}
                        ...
                        <button
                          onClick={() => update(s => ({
                            ...
                            subscriptions: s.subscriptions.map(x =>
                              x.id === sub.id
                                ? { ...x, nextDeliveryISO: new Date(Date.now() + 86400000 * 3).toISOString() }
                                : x
                            ),
                          }))}
                          className="px-3 py-1.5 rounded-full text-xs border border-ink/15 hover:bg-ink/5 flex items-center gap-1"
                        ...
                        <button
                          onClick={() => {
                            ...
                            update(s => ({
                              ...s,
                              subscriptions: s.subscriptions.filter(x => x.id !== sub.id),
                              dogs: s.dogs.filter(d => d.id !== dog.id),
                            }));
                          }}
