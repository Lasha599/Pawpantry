...
import { useStore, Subscription, addWeeks } from '@/lib/store';
...
export default function OnboardingPage() {
  ...
  const finish = () => {
    ...
    const newDog: Dog = { ...dog, id: dogId };
    const sub: Subscription = {
      id: 'sub_' + Date.now(),
      dogId,
      productId: selectedProduct.id,
      frequencyWeeks,
      nextDeliveryISO: addWeeks(new Date().toISOString(), frequencyWeeks),
      status: 'active',
    ...
    update(s => ({
      ...
      history: [
        ...s.history,
        { id: 'h1_' + dogId, dogId, productId: selectedProduct.id, deliveredAt: new Date(Date.now() - 86400000 * 3).toISOString() },
      ],
    }));
      ...
      <main className="max-w-3xl mx-auto px-6 py-12">
        ...
        <div className="flex justify-between mt-10">
          ...
          {step < 4 ? (
            ...
            <button onClick={finish} disabled={!canNext()} className="btn-primary disabled:opacity-40">
              Start subscription
            </button>
          )}
        </div>
